...
CHANGES:
  - RSI now uses Wilder's smoothed method (matches TradingView/MT4 values)
  - SL range expanded to 1000–2400 pips
  - ATR min reduced to 300p for Asian session
"""
...
class SignalEngine:
    ...
    def _calc_rsi(self, closes, period=14):
        ...
        Simple-average RSI (old method) diverges significantly from platform values.
        Wilder uses a smoothed EMA: alpha = 1/period (not 2/(period+1)).
        Requires period*2 + 1 candles for a reliable seed.
        """
        ...
        # Wilder's smoothing: each subsequent bar uses alpha = 1/period
    ...
    def _check_m15_rejection(self, direction):
        ...
        Uses the last 2 completed M15 candles and picks the more recent valid one.
        """
        try:
            ...
            # Check last 2 completed candles — bot runs every 5 min so
            # the freshest completed M15 can be up to 15 min stale.
            # If either of the last 2 shows rejection, we accept it.
            for idx in [-1, -2]:
                h = highs[idx]
                l = lows[idx]
                o = opens[idx]
    ...
    def _analyze_gold(self, is_asian=False):
        ...
        # ── CHECK 4: RSI MOMENTUM — Wilder smoothed (0–1 pt) ─
        # Using 60 H1 closes gives Wilder's method enough bars to converge properly.
